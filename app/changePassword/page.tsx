...
export default function ChangePassword() {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (!oldPassword) {
        window.showToast({
          content: t('changePassword.old_password_empty', '请输入旧密码'),
          position: "bottom",
        });
        setIsLoading(false);
        return;
      ...
      if (!confirmPassword) {
        window.showToast({
          content: t('changePassword.confirm_password_empty', '请确认您的新密码'),
          position: "bottom",
        });
        setIsLoading(false);
        return;
      ...
      if (newPassword !== confirmPassword) {
        window.showToast({
          content: t('changePassword.passwords_not_match', '两次输入的新密码不一致'),
          position: "bottom",
        });
      ...
      // 示例：const res = await changePasswordAPI(oldPassword, newPassword);
      // 模拟API调用
      const res = { success: true }; // 替换为实际的API调用结果
    ...
    <div className={styles.content}>
      ...
      <div className={styles.changePasswordContainer}>
        ...
        <div className={styles.logoSection}>
          ...
          <h2 className={styles.title}>{t('changePassword.change_password', '修改密码')}</h2>
          <p className={styles.subtitle}>{t('changePassword.subtitle', '为您的账户设置新的登录密码')}</p>
        ...
        <div className={styles.formWrapper}>
          <div className={styles.formSection}>
            <div className={`${styles.inputGroup} ${styles.passwordInput}`}>
              <Input
                placeholder={t('changePassword.old_password', '旧密码')}
                value={oldPassword}
                onChange={setOldPassword}
                onEnterPress={() => document.getElementById('newPasswordInput')?.focus()}
                clearable
                type="password"
              />
            </div>
            ...
            <div className={`${styles.inputGroup} ${styles.passwordConfirmInput}`}>
              <Input
                ...
                placeholder={t('changePassword.confirm_password_placeholder', '请再次输入新密码')}
              ...
              {confirmPassword.length > 0 && !passwordsMatch && (
                <div className={styles.passwordError}>
                  {t('changePassword.passwords_not_match', '两次输入的新密码不一致')}
                </div>
              )}
            </div>
          </div>
