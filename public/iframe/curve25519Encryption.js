...
// import { randomBytes } from 'https://unpkg.com/@stablelib/random@2.0.1/lib/random.js';  

import { randomBytes } from "https://esm.sh/@stablelib/random@2.0.1";
import * as nacl from 'https://esm.sh/tweetnacl@1.0.3';  
const { box } = nacl;
import { BigInteger } from 'https://esm.sh/jsbn@1.1.0';

export class Encryption {  
    static NONCE_LENGTH = 24;  
    static PUBLIC_KEY_LENGTH = 32;  
    static MAC_LENGTH = 16;  

    // Curve25519的素数  
    static CURVE25519_P = new BigInteger("57896044618658097711785492504343953926634992332820282019728792003956564819949");  
    ...
    static bigIntToBytes(bn, length) {  
    ...
    // 修改转换函数中的模逆计算  
    static ed25519PublicKeyToCurve25519(publicKey) {  
    ...
    static async encrypt(recipientPublicKey, message) {  
        try {  
    ...
    static async encryptWithDebug(recipientPublicKey, message) {  
        try {  
    ...
    static async ed25519PrivateKeyToCurve25519(privateKey) {  
    ...
    static async decrypt(recipientPrivateKey, encryptedData, debug = false) {  
        try {  
            if (debug) {  
    ...
    // 新增：解密调试辅助方法  
    static async debugDecryptionProcess(recipientPrivateKey, encryptedData) {  
