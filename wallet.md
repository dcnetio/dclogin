...
是否存在最近账号 -- 不存在 --> 获取dclogin列表 --> 判断是否存在已创建过的账号
...
调起webauth认证 --> 提取出userHandleHash --> 解密助记词 --> 通过助记词导入dclogin,生成带私钥dclogin账号 --> dclogin签名加密 --> 返回当前用户信息
```
...
判断需要签名的账号是否存在于数据库 -- 不存在 --> 提示当前dclogin不存在该账号
判断需要签名的账号是否存在于数据库 -- 存在 --> 调出webauthn进行校验,并提取出userHandleHash --> 解密出助记词 --> 通过助记词导入dclogin,生成带私钥dclogin账号 --> 执行签名 --> 返回签名信息
end
```
